...
void Otto9::oscillateServos(int A[4], int O[4], int T, double phase_diff[4], float cycle) {
  for(int i = 0; i < 4; i++) {
    _servo[i].setO(O[i]);
    _servo[i].setA(A[i]);
    _servo[i].setT(T);
  ...
  double ref = millis();
  for(double x = ref; x <= _T * cycle + ref; x = millis()) {
    for(int i = 0; i < 4; i++) {
      _servo[i].refresh();
    }
  }
...
void Otto9::_execute(int A[4], int O[4], int T, double phase_diff[4], float steps) {
  attachServos();
  if(getRestState() == true) {
    setRestState(false);
  }
