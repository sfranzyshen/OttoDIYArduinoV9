...
void Otto9::home() {
  if(getRestState() == false) { // Go to rest position only if necessary
    ...
    moveServos(500, (int)[90, 90, 90, 90]); //Move the servos in half a second
    detachServos();
    setRestState(true);
  }
}
...
//  int up[] = {90, 90, 150, 30};
//  moveServos(T, up);
  moveServos(T, (int)[90, 90, 150, 30]);
//  int down[] = {90, 90, 90, 90};
//  _moveServos(T, down);
  moveServos(T, (int)[90, 90, 90, 90]);
}
...
//  double phase_diff[] = {0, 0, DEG2RAD(dir * -90), DEG2RAD(dir * -90)};
  ...
  _execute((int)[30, 30, 20, 20], (int)[0, 0, 4, -4], T, (double)[0, 0, DEG2RAD(dir * -90), DEG2RAD(dir * -90)], steps);  
}
...
void Otto9::turn(float steps, int T, int dir) {
  ...
  // Let's oscillate the servos!
  _execute(A, (int)[0, 0, 4, -4], T, (double)[0, 0, DEG2RAD(-90), DEG2RAD(-90)], steps); 
}
...
void Otto9::shakeLeg(int steps, int T,int dir) {
  ...
  for(int j = 0; j < steps; j++) {
    ...
    moveServos(500, (int)[90, 90, 90, 90]); //Return to home position
  }
  delay(T);
}
...
void Otto9::updown(float steps, int T, int h) {
  ...
  _execute((int)[0, 0, h, h], (int)[0, 0, h, -h], T, (double)[0, 0, DEG2RAD(-90), DEG2RAD(90)], steps); 
}
...
void Otto9::swing(float steps, int T, int h) {
  ...
  // Let's oscillate the servos!
  _execute((int)[0, 0, h, h], (int)[0, 0, h / 2, -h / 2], T, (double)[0, 0, DEG2RAD(0), DEG2RAD(0)], steps); 
}
...
void Otto9::tiptoeSwing(float steps, int T, int h) {
  ...
  //-- Let's oscillate the servos!
  _execute((int)[0, 0, h, h], (int)[0, 0, h, -h], T, (double)[0, 0, 0, 0], steps); 
}
...
void Otto9::jitter(float steps, int T, int h) {
  ...
  // Let's oscillate the servos!
  _execute((int)[h, h, 0, 0], (int)[0, 0, 0, 0], T, (double)[DEG2RAD(-90), DEG2RAD(90), 0, 0], steps); 
}
...
void Otto9::ascendingTurn(float steps, int T, int h) {
  ...
  // Let's oscillate the servos!
  _execute((int)[h, h, h, h], (int)[0, 0, h + 4, -h + 4], T, (double)[DEG2RAD(-90), DEG2RAD(90), DEG2RAD(-90), DEG2RAD(90)], steps); 
}
...
void Otto9::moonwalker(float steps, int T, int h, int dir) {
  ...
  //int phi = (-dir * 90);
  //double phase_diff[4] = {0, 0, DEG2RAD(-dir * 90), DEG2RAD(-60 * dir + (-dir * 90))};
  
  //-- Let's oscillate the servos!
  _execute((int)[0, 0, h, h], (int)[0, 0, h / 2 + 2, -h / 2 - 2], T, (double)[0, 0, DEG2RAD(-dir * 90), DEG2RAD(-60 * dir + (-dir * 90))], steps); 
}
...
void Otto9::crusaito(float steps, int T, int h, int dir) {
  ...
  //-- Let's oscillate the servos!
  _execute((int)[25, 25, h, h], (int)[0, 0, h / 2 + 4, -h / 2 - 4], T, (double)[90, 90, DEG2RAD(0), DEG2RAD(-60 * dir)], steps); 
}
...
void Otto9::flapping(float steps, int T, int h, int dir) {
  ...
  // Let's oscillate the servos!
  _execute((int)[12, 12, h, h], (int)[0, 0, h - 10, -h + 10], T, (double)[DEG2RAD(0), DEG2RAD(180), DEG2RAD(-90 * dir), DEG2RAD(90 * dir)], steps); 
}
